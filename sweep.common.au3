#cs ----------------------------------------------------------------------------
 ...
 Script Function:
    Functions used by sweep
	Based on runLORASR.Functions v0.4.4.0
...
Global CONST $g_sCommonVersion = "0.0.1.0"
...
Global CONST $g_sSettingsFile = "sweep.ini"
Global CONST $g_asSettingsSearchPath = ["."]
Global $g_sLogFile = DateTimeFileName("sweep", "log.md")
...
Func LogVersions($sExecutable = "", $sExecutableVersion = "")
    ...
    LogMessage("Loaded `sweep.common` version " & $g_sCommonVersion, 3)
    ...
    If $g_sSweepVersion Then LogMessage("Loaded `sweep.functions` version " & $g_sSweepVersion, 3)
    ...
    If $g_sProgressVersion Then LogMessage("Loaded `sweep.progress` version " & $g_sProgressVersion, 3)
...
Func GetSettings($sWorkingDirectory, ByRef $sProgramPath, ByRef $sSimulationProgram, ByRef $sSweepFile, ByRef $sTemplateFile, ByRef $sResultsFile, ByRef $sPlotFile, ByRef $sInputFolder, ByRef $sOutputFolder, ByRef $sRunFolder, ByRef $sIncompleteFolder, ByRef $bCleanup)
    ...
    $sProgramPath = IniRead($sSettingsFile, "Files and folders", "ProgramPath", ".")
    $sSimulationProgram = IniRead($sSettingsFile, "Files and folders", "SimulationProgram", "unknown")
...
Func CreateLogFile($sLogFile = $g_sLogFile, $sWorkingDirectory = @WorkingDir)
    ...
    FileWriteLine($hLogFile, "")
    FileWriteLine($hLogFile, "# sweep log file")
    FileWriteLine($hLogFile, "")
    ...
    FileWriteLine($hLogFile, "--------------------------------------------------------------------------------")
    FileWriteLine($hLogFile, "")
    FileWriteLine($hLogFile, "## Loading sweep libraries")
    FileWriteLine($hLogFile, "")
